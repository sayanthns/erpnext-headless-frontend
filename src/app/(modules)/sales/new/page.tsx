...
export default function NewSalesInvoice() {
        ...
        async function loadData() {
            try {
                const [custRes, itemRes] = await Promise.all([
                    fetch('/api/proxy?endpoint=resource/Customer&fields=["name","customer_name"]'),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/proxy?endpoint=resource/Sales Invoice', {
                method: 'POST',
            ...
            if (res.ok) {
                alert("Sales Invoice created successfully! ID: " + data.data.name);
                router.push('/sales');
            } else {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">New Sales Invoice</h1>
                    <p className="text-gray-500 mt-2">Create a new invoice for a customer.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                ...
                {/* Items Table */}
                <h3 className="text-lg font-bold text-slate-800 mb-4 border-b pb-2">Items</h3>
                    ...
                    {items.map((item, index) => (
                        <div key={index} className="flex gap-4 items-end">
                            ...
                            <div className="w-32">
                                ...
                                <input type="text" readOnly value={item.rate} className="w-full px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-500 cursor-not-allowed" />
                            </div>
                            <div className="w-32">
                ...
                <div className="flex justify-end gap-4">
                    <button type="button" onClick={() => router.push('/sales')} className="px-6 py-2 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50">
                        Cancel
                    </button>
